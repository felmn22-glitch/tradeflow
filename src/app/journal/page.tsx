    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm">{state.trades.length} operações registradas</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={downloadTemplate} className="border-gray-600 text-gray-300">
                  ...
                  <div>
                    ...
                    <Select value={form.type} onValueChange={(v) => setForm({ ...form, type: v as TradeType })}>
                      ...
                      <SelectContent className="bg-gray-700 border-gray-600">
                        ...
                        <SelectItem value="forex">Forex (Ganho de Capital)</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  <div>
        ...
        {filtered.length === 0 ? (
          <Card className="bg-gray-800/50 border-gray-700">
            <CardContent className="py-12 text-center">
              ...
              <p className="text-gray-500 text-sm mt-1">Clique em "Nova Operação" ou importe um CSV.</p>
            </CardContent>
          </Card>
        ) : (
          filtered.map((trade) => (
  ...
  return (
    <Card className="bg-gray-800/50 border-gray-700">
      <CardContent className="p-0">
        <div
          className="flex items-center gap-4 px-4 py-3 cursor-pointer hover:bg-gray-700/30 transition-colors"
          onClick={onToggle}
        >
          <div className="flex-1 flex items-center gap-3 min-w-0">
            <span className="font-mono font-bold text-white w-16">{trade.asset}</span>
            <Badge
              variant="outline"
              className={`text-xs shrink-0 ${trade.type === "day_trade" ? "border-blue-500/50 text-blue-400" : "border-purple-500/50 text-purple-400"}`}
            >
              {trade.type === "day_trade" ? "Day Trade" : "Swing"}
            </Badge>
            <Badge
              variant="outline"
              className={`text-xs shrink-0 ${trade.direction === "compra" ? "border-emerald-500/50 text-emerald-400" : "border-orange-500/50 text-orange-400"}`}
            >
              {trade.direction === "compra" ? "Compra" : "Venda"}
            </Badge>
            <span className="text-gray-400 text-xs shrink-0">{trade.date}</span>
            {trade.notes && (
              <span className="text-gray-500 text-xs truncate hidden sm:block">{trade.notes}</span>
            )}
          </div>
        ...
        {expanded && (
          <div className="px-4 pb-4 border-t border-gray-700/50 pt-3">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
              <Detail label="Quantidade" value={trade.quantity.toString()} />
